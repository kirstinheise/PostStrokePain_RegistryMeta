...
'finalfit', 'flextable', 'likert', 'scales', 'here', 'kableExtra', 'flextable', 'magrittr', 'correlation', "see")
...
noPain_sum <- unique.data.frame( psp_dat6.df[, c("Deidentified.ID", "subjType", "noArmPain")]) %>%
...
data <- psp_dat1.df[, c("Deidentified.ID", "affectedArmPain", "time.post.stroke", "Gender", "Race", "LesionHemisphere", "age.at.stroke", "age.at.assessment")] %>%
...
# recalculate time.post.stroke into months
data$months.post.stroke <- data$time.post.stroke/30
...
explanatory = c("months.post.stroke", "age.at.stroke", "Gender", "LesionHemisphere", "Race") #, "Ethnicity")
...
# 
# p +
#   scale_y_log10(                           # <-- key change (y, not x)
#     limits = c(x_lo, x_hi),
#     breaks = c(0.5, 1, 2, 5, 10),
#     oob = scales::squish,                  # caps whiskers at the limits
#     expand = expansion(mult = c(0.05, 0.15))
#   ) +
#   coord_flip(clip = "off")                # keep the flip; don't use coord_cartesian here
#   #+ theme(plot.margin = margin(t = 8, r = 12, b = 8, l = 40))
...
m3new <- glm(affectedArmPain ~ months.post.stroke,
          data = data,
          family = "binomial")
...
plot_model(m3new, type = "pred", terms = "months.post.stroke [all]") + labs(y = ("p[affected arm pain]"), x = "months post stroke")

m7new <- glm(affectedArmPain ~ Gender + months.post.stroke + age.at.assessment + LesionHemisphere + Race ,
          data = data, 
          family = "binomial")

tab_model(m7new)

plot_model(m7new, type = "pred", terms = c("months.post.stroke [all]", "Gender", "LesionHemisphere")) + labs(y = ("p[affected arm pain]"), x = "months post stroke")
